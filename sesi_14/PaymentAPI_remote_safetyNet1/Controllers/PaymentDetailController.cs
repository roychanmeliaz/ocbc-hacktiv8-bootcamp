...
namespace TodoApp.Controllers
{
    ...
    public class PaymentDetailController : ControllerBase
    {
        ...
        {
            var item = await _context.PaymentDetails.FirstOrDefaultAsync(x => x.paymentDetailId == paymentDetailId);
            if (item == null)
                return NotFound();
            return Ok(item);
        }
        ...
        public async Task<ActionResult> UpdateItem(int paymentDetailId, ItemData item)
        {
            ...
            var existItem = await _context.PaymentDetails.FirstOrDefaultAsync(x=>x.paymentDetailId == paymentDetailId);
            if (existItem == null)
                return NotFound();
